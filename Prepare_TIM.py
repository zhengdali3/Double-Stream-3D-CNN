...
def prepareSAMM(MM, OF, fps, SAMM_dataset_path):
            ...
            if (dataSheet.loc[sample, 'Duration'] >= image_depth and emotion != 'Other'):
                ...
                for image in os.listdir(SAMM_dataset_path + '/SAMM/'+ subject + '/' + sample + '/'):
                    ...
                    if(imagecode >= start and imagecode < end):
                        ...
                        frames.append(img)
                        temp = fa.face_align(img, 960, 640)
                ...
                video_tensor = tim.TIM(video_tensor, 18)
    ...
    np.save('SAMM/SAMM_set_TIM_FA.npy', SAMM_set)
...
def prepareCK(MM, OF, fps, CK_dataset_path):
    ...
    for sub_ind, subject in enumerate(os.listdir(label_path)):
        subject_label_path = os.path.join(label_path, subject)
        subject_image_path = os.path.join(image_path, subject)
        print("CK+ current subject: ", subject, " ", len(os.listdir(label_path)) - sub_ind, " subject left.")
            ...
            if os.listdir(test_label_path):
                ...
                if label != 0:
                    ...
                    if (apex_num < image_depth):
                        ...
                        ori_depth = image_depth

                    video_tensor = np.zeros((ori_depth,480,640,3),dtype='float')
                    
                    for f in range(ori_depth):
                        image = cv2.imread(test_image_path + '/' + image_dir[f])
                        frames.append(image)
                        temp = image[:480,:640,:]
#                         temp = fa.face_align(image, 640, 480)
                        video_tensor[f] = temp
                    ...
                    if(OF == True):
                        frames_flow = []
                        for i in range(18):
                            frame_resize = cv2.resize(video_tensor[i][:][:][:], (flow_columns, flow_rows), interpolation = cv2.INTER_AREA)
                            frame_flow = np.float32(frame_resize)
                            frames_flow.append(frame_flow)
...
#     CK_flow_set /= np.max(CK_flow_set)
    ...
    np.save('CK+/CK_set_TIM_FA.npy', CK_set)
    ...
    for subject_path in directorylisting:
        ...
        for label in range(3):
            ...
            for example in os.listdir(path[label]):
                ...
                video_tensor = np.zeros((framerange, 144, 120, 3), dtype='float')                
                
#                 j = 0
#                 while(j < len(framelisting) and i < framerange):
                    
#                     imagepath = example_path + "/" + framelisting[j]
#                     image = cv2.imread(imagepath)
#                     temp = fa.face_align(image, 120, 144)
#                     if(temp is not None):
#                         video_tensor[i] = temp
#                         i = i + 1
#                         frames.append(image)
#                         j = j + 1
#                     else:
#                         print("Current img: ", imagepath)
#                         j = j + 1
                        
#                 if(i < framerange):
                i = 0
                while(i < framerange):
                    imagepath = example_path + "/" + framelisting[i]
                    image = cv2.imread(imagepath)
                    temp = image[:144, :120,:]
                    video_tensor[i] = temp
                    i = i + 1
    ...
    np.save('SMIC/SMIC_set_TIM_FA.npy', SMIC_set)
